...
export const UserProvider = ({
  ...
  useEffect(() => {
    ...
    async function fetchClient() {
      ...
      try {
        const res = await axios.get(`/api/admin/clients/${userId}`, { signal });
        if (res.status !== 200) {
          throw new Error(
            `Failed to fetch client: ${res.status} ${res.statusText}`
          );
        }
        ...
        if (err.name !== "AbortError") {
          setError(err.message || "Unknown error fetching client");
        }
      } finally {
        setLoading(false);
      }
  ...
  const updateClient = async (
    ...
    // Example: fetch(`/api/clients/${userId}`, { method: 'PATCH', body: JSON.stringify(data) })
    if (runDB) {
      try {
        const res = await axios.post(`/api/admin/clients/${userId}`, data);
        if (res.data.success) {
          console.log("Client updated successfully");
        } else {
          console.error("Failed to update client:", res.data.error);
        }
      } catch {}
    }
  };
